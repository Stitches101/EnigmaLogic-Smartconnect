...
//**********************************************************************************************************************************************//
//Main Function - EL04_01: TCP Server Connection Maintenance
//**********************************************************************************************************************************************//


static TCPS_SRV *tcpsmai = NULL;
static TCP_STREAM *tcpcmai = NULL;


void com3_libsuper000(COM3_LIBSUPER000 *io)
{
//Variable Input
	//**************************************************************************//
	BIT in01 = 0;
...
if (in01 > 0 && tcpsmai == 0)
{
	...
	tcpsmai = TCPSSRV_CreateServer(poort);//TCPS_SRV*   TCPSSRV_CreateServer    (int iPort);

}
else if (tcpsmai != 0)
{
	if (TCPSSRV_AcceptRequested(tcpsmai))
	{
		...
		*io->__s04 = 0;
		...
		tcpcmai = TCPSSRV_Accept(tcpsmai, 10000);// TCP_STREAM* TCPSSRV_Accept(TCPS_SRV* pTCPSSRV, int iTimeoutMS);

		if (tcpcmai != 0)
		{
			if (TCPS_ConnectNblkComplete(tcpcmai) == 0)
			{
				if (TCPS_GetRemotePort(tcpcmai) != 0)
				{
					*io->__s01 = 5;
				}
			}
	...
	if (*io->__s01 == 5)
	{
		*io->__s02 = sizeof(buf1) + 1;
		bytesIn = TCPS_ReceiveData(tcpcmai, buf3, sizeof(buf3) + 1, 1000);
		...
		*io->__s01 == 0;
		...
		if (Count)
		{
			...
			while ((pst = *ppst) != NULL)
			{
				bytesSafe = TCPS_RequestSigIo(tcpcmai);
				if (
					(*pst->Funcid == 2 && *io->__s02 != 99 && bytesSafe == aFALSE)
					)
				{
					*io->__s02 = 99;
					...
					if (Count02 == 20)
					{
						...
						*io->__s02 = sizeof(buf1) + 1;
						...
						if (bytesIn > 0)
						{
							while (*io->__s02 > 0)
							{
								bytesOut = TCPS_SendData(tcpcmai, buf1, sizeof(buf1) + 1);
								*io->__s03 = bytesOut;
								*io->__s02 -= *io->__s03;
							}
						}
					...
					{
						TCPSSRV_RemoveServer(&tcpsmai);
						TCPS_Disconnect(&tcpcmai);
						memset(buf1, 0, sizeof(buf1));
						memset(buf2, 0, sizeof(buf2));
						memset(buf3, 0, sizeof(buf3));
...
{
	TCPSSRV_RemoveServer(&tcpsmai);//void        TCPSSRV_RemoveServer    (TCPS_SRV** ppTCPSSRV);
	TCPS_Disconnect(&tcpcmai);
}
