...
//**********************************************************************************************************************************************//
//Main Function - EL04_01: TCP Server Connection Error (malfunction)
//**********************************************************************************************************************************************//
static TCPS_SRV *tcpserr = NULL;
static TCP_STREAM *tcpcerr = NULL;


void com2_libsuper000(COM2_LIBSUPER000 *io)
{
	//Variable Input
	//**************************************************************************//
	BIT in01 = 0;
	...
	if (in01 > 0 && tcpserr == 0)
	{
		...
		tcpserr = TCPSSRV_CreateServer(poort);//TCPS_SRV*   TCPSSRV_CreateServer    (int iPort);

	}
	else if (tcpserr != 0)
	{
		if (TCPSSRV_AcceptRequested(tcpserr))
		{
			...
			*io->__s04 = 0;
			...
			tcpcerr = TCPSSRV_Accept(tcpserr, 10000);// TCP_STREAM* TCPSSRV_Accept(TCPS_SRV* pTCPSSRV, int iTimeoutMS);

			if (tcpcerr != 0)
			{
				if (TCPS_ConnectNblkComplete(tcpcerr) == 0)
				{
					if (TCPS_GetRemotePort(tcpcerr) != 0)
					{
						*io->__s01 = 5;
					}
				}
		...
		if (*io->__s01 == 5)
		{
			*io->__s02 = sizeof(buf1) + 1;
			bytesIn = TCPS_ReceiveData(tcpcerr, buf3, sizeof(buf3) + 1, 1000);
			...
			*io->__s01 == 0;
			...
			if (Count)
			{
				...
				while ((pst = *ppst) != NULL)
				{
					...
					bytesSafe = TCPS_RequestSigIo(tcpcerr);
					if (
						(*pst->Funcid == 3 && *io->__s02 != 99 && bytesSafe == aFALSE)
						)
					{
						*io->__s02 = 99;
						...
						if (Count02 == 20)
						{
							...
							*io->__s02 = sizeof(buf1) + 1;
							...
							if (bytesIn > 0)
							{
								while (*io->__s02 > 0)
								{
									bytesOut = TCPS_SendData(tcpcerr, buf1, sizeof(buf1)+1);
									*io->__s03 = bytesOut;
									*io->__s02 -= *io->__s03;
								}
							}
						...
						{
							TCPSSRV_RemoveServer(&tcpserr);
							TCPS_Disconnect(&tcpcerr);
							memset(buf1, 0, sizeof(buf1));
							memset(buf2, 0, sizeof(buf2));
							memset(buf3, 0, sizeof(buf3));
	...
	{
		TCPSSRV_RemoveServer(&tcpserr);//void        TCPSSRV_RemoveServer    (TCPS_SRV** ppTCPSSRV);
		TCPS_Disconnect(&tcpcerr);
	}
